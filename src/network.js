...
export class Network {
  constructor() {
    ...
    // Cached encoder/decoder — avoids new allocation per message
    this._textEncoder = new TextEncoder();
    this._textDecoder = new TextDecoder();
    // Pre-allocated position send buffer (0x02 message, max name 32 bytes)
    ...
    this.onDm = null;            // (from, text, offline, id) => {}
    this.onDmOfflineCount = null; // (count) => {}
    this.onDmStatus = null;       // (id, status, from) => {}
    this.onDmSyncPull = null;     // (threads) => {} — cross-device DM sync
  ...
  _handleMessage(msg) {
    switch (msg.type) {
      ...
      case 'chat':
        ...
        break;
      case 'dm':
        if (this.onDm) this.onDm(msg.from, msg.text, msg.offline, msg.id);
        break;
      case 'dm_offline_count':
        if (this.onDmOfflineCount) this.onDmOfflineCount(msg.count);
        break;
      case 'dm_status':
        if (this.onDmStatus) this.onDmStatus(msg.id, msg.status, msg.from);
        break;
      case 'dm_sync_pull':
        if (this.onDmSyncPull) this.onDmSyncPull(msg.threads);
        break;
      case 'room_list':
  ...
  // ── Direct messages ────────────────────────────────────────────────
  sendDm(to, text, id) { this._send({ type: 'dm', to, text, id }); }
  sendDmRead(from, id) { this._send({ type: 'dm_read', from, id }); }
  sendDmSyncPush(threads) { this._send({ type: 'dm_sync_push', threads }); }
