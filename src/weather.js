...
export class WeatherSystem {
  ...
  _updateThunder(playerPos, dt) {
    ...
    const d = this.lightningPos.distanceTo(playerPos);
    const intensity = Math.max(0.2, 1 - d / 32) * (0.6 + Math.random() * 0.4);
    this.thunderFlashIntensity = intensity;

    window.dispatchEvent(new CustomEvent('weather-thunder', {
      detail: { delay: d / 343, intensity, x: this.lightningPos.x, z: this.lightningPos.z }
    }));
  }

  _spawnSplash(x, y, z) {
    const geo = new THREE.BoxGeometry(0.03, 0.03, 0.03);
    const mat = new THREE.MeshBasicMaterial({ color: 0x88aacc, transparent: true, opacity: 0.5 });
