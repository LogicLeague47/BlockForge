...
// Cached TNT textures — created once, reused across all LitTnt instances
let _tntTexCache = null;

function _getTntTexCache(atlasCanvas) {
  if (_tntTexCache && _tntTexCache.atlas === atlasCanvas) return _tntTexCache;
  const norm = {
    side: _atlasTexStatic(atlasCanvas, tileNameFor(BLOCK.TNT, 'side')),
    top: _atlasTexStatic(atlasCanvas, tileNameFor(BLOCK.TNT, 'top')),
    bot: _atlasTexStatic(atlasCanvas, tileNameFor(BLOCK.TNT, 'bottom')),
  };
  const lit = {
    side: _litVersionStatic(norm.side),
    top: _litVersionStatic(norm.top),
    bot: _litVersionStatic(norm.bot),
  };
  _tntTexCache = {
    atlas: atlasCanvas,
    texNorm: [norm.side, norm.side, norm.top, norm.bot, norm.side, norm.side],
    texLit: [lit.side, lit.side, lit.top, lit.bot, lit.side, lit.side],
  };
  return _tntTexCache;
}

function _atlasTexStatic(atlasCanvas, name) {
  const t = TILES[name];
  ...
  ctx.drawImage(atlasCanvas, t[0] * 32, t[1] * 32, 32, 32, 0, 0, 16, 16);
...
function _litVersionStatic(src) {
  if (!src) return null;
  const c = document.createElement('canvas');
  c.width = 16; c.height = 16;
  const ctx = c.getContext('2d');
...
export class LitTnt {
  constructor(scene, atlasCanvas, x, y, z, fuseTime = 1.5) {
    ...
    // Use cached textures for all LitTnt instances
    const cache = _getTntTexCache(atlasCanvas);
    this._texNorm = cache.texNorm;
    this._texLit = cache.texLit;
