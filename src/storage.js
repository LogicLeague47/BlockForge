...
function oneblockListKey() { return 'mc-clone-oneblock-worlds'; }
...
export function getOneBlockWorldList() {
  try {
    const raw = localStorage.getItem(oneblockListKey());
    return raw ? JSON.parse(raw) : [];
  } catch { return []; }
}

export function saveOneBlockWorldList(list) { _saveList(oneblockListKey(), list); }
...
export function createWorld(name, seed, gamemode, difficulty, opts = {}) {
  ...
  } else if (opts.oneblock) {
    const list = getOneBlockWorldList();
    list.unshift(world);
    saveOneBlockWorldList(list);
  } else {
...
export function deleteWorld(id, dev = false, parkour = false, oneblock = false) {
  if (oneblock) {
    const list = getOneBlockWorldList().filter(w => w.id !== id);
    saveOneBlockWorldList(list);
  } else if (parkour) {
