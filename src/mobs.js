...
// Pre-load mob textures from PNGs (generated by build-textures.mjs).
// Returns a promise that resolves when all PNGs are loaded.
// Any missing textures will be generated procedurally at mesh-build time.
const MOB_PARTS = ['body', 'head', 'leg', 'snout', 'arm', 'cape'];
function _loadMobPng(typeName, part, idx) {
  return new Promise(resolve => {
    ...
    img.src = '/textures/mobs/' + typeName + '_' + part + '_' + idx + '.png';
  });
}

export function preloadMobTextures() {
  const promises = [];
  for (const [typeName, def] of Object.entries(MOB_TYPES)) {
    if (def.isPlayerSkin || def._tex) continue;
    const faces = {};
    let ok = true;
    for (const part of MOB_PARTS) {
      const partFaces = [];
      for (let i = 0; i < 6; i++) {
        partFaces.push(_loadMobPng(typeName, part, i));
      }
      faces[part] = Promise.all(partFaces).then(arr => {
        const loaded = arr.filter(Boolean);
        return loaded.length === 6 ? loaded : null;
      });
    }
    promises.push(
      Promise.all(Object.entries(faces).map(([k, p]) => p.then(v => [k, v]))).then(entries => {
        const result = {};
        for (const [k, v] of entries) { if (v) result[k] = v; }
        if (Object.keys(result).length > 0) def._tex = result;
      })
    );
  }
