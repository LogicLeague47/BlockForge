...
export class RemotePlayer {
  constructor(scene, name, skinIndex, x, y, z, cgUsername, teamColor) {
    this.name = name;
    this.cgUsername = cgUsername || '';
    this.skinIndex = skinIndex || 0;
    this.role = 'player';
    this.visible = true;
    this.teamColor = teamColor || null;
    ...
    this.nameTag = this._createNameTag(name, cgUsername, teamColor);
  ...
  _createNameTag(name, cgUsername, teamColor) {
    ...
    // Bedwars team color bar across the top of the tag
    if (teamColor) {
      ctx.fillStyle = teamColor;
      ctx.fillRect(4, 4, 248, 6);
    }
  ...
  _updateModel(dt, crouching) {
    ...
    if (!this._dummyPos) this._dummyPos = { x: 0, y: 0, z: 0 };
    this._dummyPos.x = this.x;
    this._dummyPos.y = drawY;
    this._dummyPos.z = this.z;
    if (!this._dummyLook) this._dummyLook = { x: 0, z: 0, y: 0 };
    this.model.update(dt, this._dummyPos, this.yaw, this._dummyLook, true, false, false, false, false);
...
export class MultiplayerRenderer {
  ...
  addPlayer(name, skinIndex, x, y, z, role, cgUsername, teamColor) {
    if (this.remotePlayers.has(name)) return this.remotePlayers.get(name);
    const rp = new RemotePlayer(this.scene, name, skinIndex, x, y, z, cgUsername, teamColor);
